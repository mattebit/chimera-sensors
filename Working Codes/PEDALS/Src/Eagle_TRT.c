...
float LSMD9S0_read(imu_stc *imu)
{
	...
	if (axis > 32767)
	{ ///Generating positive and negative value of rotation
		axis = axis - 65536;
	}
...
double read_encoder(enc_stc *enc)
{
	...
	for (int i = 0; i < clock_loop; i++)
	{
		...
		if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_8) == GPIO_PIN_SET && i <= 15)
		...
		if (i == clock_loop - 1)
		{ //if it is the last loop set the clock pin to high, else set to low
		...
		if (i == 15)
		{ //if it is the last loop cast the sata from binary to decimal
...
void encoder_tim_interrupt(enc_stc *enc)
{
	...
	{ //every 3 times request the angle from encoder
	...
	else
	{
		...
		else
		{
			...
			{ //calculate the speed from the two last angles
				enc->refresh = 1000;
				enc->wheel_diameter = 0.4064;
				double Speed = get_speed_encoder(enc);
				if (abs(Speed - enc->speed[8]) <= abs(enc->speed[8] * 10))
				{ //exclude the wrong speeds
